...
pub fn single_backtic(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
...
pub fn single_tilde(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("~").parse(input)?;
let (input, _) = not(tag("~")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_exclamation(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("!").parse(input)?;
let (input, _) = not(tag("!")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_at_sing(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("@").parse(input)?;
let (input, _) = not(tag("@")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_octothorpe(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("#").parse(input)?;
let (input, _) = not(tag("#")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_dollar_sign(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("$").parse(input)?;
let (input, _) = not(tag("$")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_percent_sign(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("%").parse(input)?;
let (input, _) = not(tag("%")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_caret(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("^").parse(input)?;
let (input, _) = not(tag("^")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_ampersand(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("&").parse(input)?;
let (input, _) = not(tag("&")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_astrisk(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("*").parse(input)?;
let (input, _) = not(tag("*")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_open_paren(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("(").parse(input)?;
let (input, _) = not(tag("(")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_close_paren(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag(")").parse(input)?;
let (input, _) = not(tag(")")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_open_curly_bracket(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("{").parse(input)?;
let (input, _) = not(tag("{")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_close_curly_bracket(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("}").parse(input)?;
let (input, _) = not(tag("}")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_open_bracket(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("[").parse(input)?;
let (input, _) = not(tag("[")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_close_bracket(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("]").parse(input)?;
let (input, _) = not(tag("]")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_less_than(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("<").parse(input)?;
let (input, _) = not(tag("<")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_greater_than(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag(">").parse(input)?;
let (input, _) = not(tag(">")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_colon(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag(":").parse(input)?;
let (input, _) = not(tag(":")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_pipe(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("|").parse(input)?;
let (input, _) = not(tag("|")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_underscore(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
...
pub fn single_hyphen(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("-").parse(input)?;
let (input, _) = not(tag("-")).parse(input)?;
  Ok((input, result))
  }
...
pub fn single_equal_sign(input: &str) -> IResult<&str, &str, VerboseError<&str>> {
let (input, result) = tag("=").parse(input)?;
let (input, _) = not(tag("=")).parse(input)?;
  Ok((input, result))
  }
