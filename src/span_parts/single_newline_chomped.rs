...
// REMINDER: This looks for a single newline and
// returns an empty string (i.e. there is no space
// returned)

pub fn single_newline_chomped(input: Text) -> IResult<Text, Text> {
...
mod tests {
  ...
  #[case(
    "single newline with content after it returns empty string",
    "\nx",
    "",
    "x"
  )]
  #[case(
    "single newline with spaces then content after it returns emptystring and chomps whitespace on the next line",
    "\n  x",
    "",
    "x"
  )]
  ...
  fn single_newline_chomped_test_runner(
