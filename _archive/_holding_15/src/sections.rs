...
pub fn sections(
  ...
  let (source, results) = many0(alt((
    ...
    alt((
      aside, blockquote, closediv, code, css, endcode,
      hidden, html, hr, list, image, opendiv, olist, pre,
      script, startcode, title, vimeo, youtube,
    )),
    alt((h, p)),
  )))(source)?;
  Ok((source, results))
  ...
  pub fn basic_integration() {
    ...
    let expected = vec![
      ...
      Section::Aside {
        ...
        paragraphs: vec![Block::Paragraph {
          tags: vec![Tag::Text {
            text: "Add salt before you fry the egg"
              .to_string(),
          }],
        }],
      },
      Section::Blockquote {
        ...
        paragraphs: vec![Block::Paragraph {
          tags: vec![Tag::Text {
            text: "Hang tinsel from both branches"
              .to_string(),
          }],
        }],
      },
